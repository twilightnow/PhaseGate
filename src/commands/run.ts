...
import { ClaudeRunner } from '../core/ai-runner';
...
async function runPhase3(
  ...
  console.log(chalk.cyan('→') + ' Launching Orchestrator for Phase 3 (parallel module dev)...');
...
async function runSinglePhase(cwd: string, phase: PhaseId): Promise<void> {
  ...
  try {
    // Phase 0 (requirements discussion) is interactive — use chat()
    if (phase === 0) {
...
function printOrchestratorResults(results: ModuleRunResult[]): void {
  ...
  for (const r of failed) {
    console.log(`  ${chalk.red('✗')} ${r.moduleName} — ${r.error ?? 'failed'}`);
  }
